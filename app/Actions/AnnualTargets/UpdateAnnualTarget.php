...
final class UpdateAnnualTarget
{
    ...
    public function execute(int $indicatorId, int $rowId, int $userId, string $activity, string $category, array $rows): bool
    {
        return DB::transaction(function () use ($indicatorId, $rowId, $userId, $activity, $category, $rows): bool {
            $owned = DB::table('ipc_targets_indicators_itemlist as itl')->join('ipc_targets_indicators as iti', 'itl.ind_id', '=', 'iti.id')->where('itl.id', $rowId)->where('iti.id', $indicatorId)->where('iti.user_id', $userId)->exists();
            if (! $owned) return false;
            foreach ($rows as $itemId => $values) {
                DB::table('ipc_targets_indicators_itemlist')->where('id', $itemId)->where('ind_id', $indicatorId)->update([
            ...
            DB::table('ipc_targets_indicators')->where('id', $indicatorId)->where('user_id', $userId)->update(['activity' => $activity, 'kra_category' => $category]);
            return true;
        });
    }
}
