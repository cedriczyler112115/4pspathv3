        ...
        DB::transaction(function () use ($submittedRows, $userId, $validated): void {
            ...
            if ($this->creatingSubTarget) {
                foreach ($this->pendingSubTargets as $pendingSubTarget) {
                    $nextItemOrder = ((int) DB::table('ipc_targets_indicators_itemlist')
                        ->where('ind_id', $this->indicatorId)
                        ->max('display_order')) + 1;

                    $insertedId = (int) DB::table('ipc_targets_indicators_itemlist')->insertGetId([
                    ...
                    if ($insertedId > 0) {
                        $newRow = DB::table('ipc_targets_indicators_itemlist as itl')
