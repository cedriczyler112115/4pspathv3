...
export default function PlRating({
  ...
  const filterPageKey = `rpmo-pl-rating-${rating.id}`;
    ...
    initialIndicators.forEach((g) => {
      g.items.forEach((item) => {
        const baseline = {
          ...
          movs: formatTextValue(item.movs, ''),
          remarks: formatTextValue(item.scorecardRemarks ?? '', ''),
          actEfficiency: item.scorecardEfficiency !== null && item.scorecardEfficiency !== undefined ? String(item.scorecardEfficiency) : '',
          actQuality: item.scorecardQuality !== null && item.scorecardQuality !== undefined ? String(item.scorecardQuality) : '',
          actTimeliness: item.scorecardTimeliness !== null && item.scorecardTimeliness !== undefined ? String(item.scorecardTimeliness) : '',
        };
  ...
  const saveField = (itemId: number, field: string, value: string) => {
    ...
    router.patch(
      `/rpmo-management/pls-scorecard/${rating.id}/accomplishment/${itemId}`,
      updated,
      {
        preserveScroll: true,
        preserveState: true,
  ...
  return (
    <AppLayout appName={appName} user={user} sidebar={navigation?.sidebar ?? []}>
      <Head title={`Provincial Link Scorecard - ${rating.year}`} />
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-border bg-card p-3 sm:p-4 shadow-2xs space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/80 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-sm font-bold tracking-tight text-foreground flex items-center gap-2">
                  <span>Provincial Link Scorecard</span>
            ...
            <div>
              <Link
                href="/rpmo-management/pls-scorecard"
              ...
              >
                ...
                <span>Back to PL Scorecard</span>
              </Link>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4 min-h-[350px]">
            ...
            {activeTab === 'performance' && (
              <div className="space-y-3">
                ...
                <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4 items-end pb-3 border-b border-border/80">
                                          ...
                                          <td className="px-3 py-3 align-top text-xs text-foreground">
                                            {isReadOnly ? (
                                              ...
                                              <AutoResizingTextarea
                                                ...
                                                onChange={(e) => handleItemFieldChange(item.itemId, 'remarks', e.target.value)}
                                                onBlur={(e) => saveField(item.itemId, 'remarks', e.target.value)}
                                                placeholder="Enter scorecard remarks..."
                                                className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring shadow-inner"
                                              />
                                            )}
                                          </td>
