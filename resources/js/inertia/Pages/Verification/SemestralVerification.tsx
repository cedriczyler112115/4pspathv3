...
export default function SemestralVerification({
  appName,
  user,
  rating,
  userProfile = {},
  ...
  // Rich Text Supervisor Feedback state
  const [recommendationHtml, setRecommendationHtml] = useState(rating.recommendation || '');
  const [strengthsHtml, setStrengthsHtml] = useState(rating.strengths || '');
  ...
  const [confirmModal, setConfirmModal] = useState<{
    ...
    icon?: 'lock' | 'unlock' | 'trash' | 'restore' | 'info' | 'verify';
    onConfirm: () => void;
  } | null>(null);
  ...
  // Sync Rich Text feedback if rating changes
  useEffect(() => {
    setRecommendationHtml(rating.recommendation || '');
  }, [rating.recommendation]);

  useEffect(() => {
    setStrengthsHtml(rating.strengths || '');
  }, [rating.strengths]);

  // Handler to verify rating with confirm modal
  const handleVerifyRating = () => {
    setConfirmModal({
      isOpen: true,
      title: 'Verify Semestral Performance Rating',
      message: 'Are you sure you want to verify and approve this staff semestral performance rating? This will record the official verification timestamp in Asia/Manila and mark this rating as Verified.',
      confirmText: 'Yes, Verify Rating',
      cancelText: 'Cancel',
      variant: 'emerald',
      icon: 'verify',
      onConfirm: () => {
        router.post(
          ...
          {
            preserveScroll: true,
            onSuccess: () => toast.success('Staff semestral performance rating verified successfully!'),
          }
        );
      },
    });
  ...
  // Handler to unverify rating with confirm modal
  const handleUnverifyRating = () => {
    setConfirmModal({
      isOpen: true,
      title: 'Revert Verification Status',
      message: 'Are you sure you want to unverify this semestral rating? This will reset the verification status, verifier ID, and timestamps on this rating, its indicators, and accomplishment item lists back to unverified.',
      confirmText: 'Yes, Unverify Rating',
      cancelText: 'Cancel',
      variant: 'warning',
      icon: 'restore',
      onConfirm: () => {
        router.post(
          ...
          {
            preserveScroll: true,
            onSuccess: () => toast.success('Verification status reverted.'),
          }
        );
      },
    });
  ...
  // Save Supervisor Feedback (Recommendations & Strengths)
        ...
        onSuccess: () => {
          toast.success(
            target === 'recommendation'
              ? "Rater's comments & recommendations saved!"
              : target === 'strengths'
              ? 'Strengths saved successfully!'
              : 'Supervisor feedback updated successfully!'
          );
        },
        onError: () => toast.error('Failed to save supervisor feedback. Please try again.'),
        onFinish: () => setIsSavingFeedback(false),
      }
    );
  };

  // Inline Development Plan: Save New Item
  const handleSaveNewArea = () => {
    if (!newAreaForm.areas_improvement.trim() || !newAreaForm.development_activities.trim() || !newAreaForm.support_resources.trim()) {
      toast.error('Please fill in all required development plan fields.');
      return;
    }
    ...
    router.post(
      ...
      newAreaForm,
      {
        ...
        onSuccess: () => {
          ...
          toast.success('Development plan item added successfully.');
        },
        onError: () => toast.error('Failed to add development plan item.'),
        onFinish: () => setIsSavingArea(false),
      }
    );
  };

  // Inline Development Plan: Save Edit Item
  const handleSaveEditArea = (areaId: number) => {
    if (!editAreaForm.areas_improvement.trim() || !editAreaForm.development_activities.trim() || !editAreaForm.support_resources.trim()) {
      toast.error('Please fill in all required development plan fields.');
      return;
    }
    setIsSavingArea(true);
    router.put(
      ...
      editAreaForm,
      {
        ...
        onSuccess: () => {
          ...
          toast.success('Development plan item updated successfully.');
        },
        onError: () => toast.error('Failed to update development plan item.'),
        onFinish: () => setIsSavingArea(false),
      }
    );
  };

  // Inline Development Plan: Delete Item
  ...
  return (
    <AppLayout appName={appName} user={user} sidebar={navigation?.sidebar ?? []}>
      <Head title={`Semestral Target Verification - ${rating.year}`} />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/80 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-sm font-bold tracking-tight text-foreground flex items-center gap-2">
                  <span>Staff Semestral Performance Verification</span>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Review and verify staff semestral target commitments, accomplishments, ratings, documentation, and checkpoint changes.
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              >
                ...
                <span>Back to Staff Verification</span>
              </Link>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4 min-h-[350px]">
                                        ...
                                        >
                                          ...
                                          {isLocked && (
                                            <td className="px-3 py-3 border-r border-border align-top text-xs h-1 relative">
                                              ...
                                              <div className="flex h-full min-h-[120px] flex-col justify-between">
                                                <div className={`space-y-2 text-xs text-foreground leading-normal whitespace-pre-line pb-2 ${Boolean(item.verified) ? 'pr-6' : ''}`}>
                                                  <FormattedText value={itemValues[item.itemId]?.actualAccomplishment || item.actualAccomplishment || '—'} />
                                                </div>
                                          ...
                                          <td className="px-3 py-3 border-r border-border align-top text-xs text-foreground">
                                            <div className="text-xs text-foreground leading-normal whitespace-pre-line">
                                              <FormattedText value={itemValues[item.itemId]?.movs || item.movs || '—'} />
                                            </div>
                                          </td>
                                          ...
                                          <td className="px-3 py-3 align-top text-xs text-foreground">
                                            <div className="text-xs text-foreground leading-normal whitespace-pre-line">
                                              <FormattedText value={itemValues[item.itemId]?.remarks || item.remarks || '—'} />
                                            </div>
                                          </td>
                                        </tr>
                                      );
            ...
            {activeTab === 'feedback' && (
              <div className="space-y-6">
                {/* Status Banner */}
                {!isVerified ? (
                  <div className="rounded-2xl border border-amber-200/80 bg-amber-50/70 dark:border-amber-900/50 dark:bg-amber-950/30 p-6 flex flex-col items-center justify-center text-center space-y-3 shadow-2xs">
                    <div className="p-3 rounded-full bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400 shadow-xs">
                      <MessageSquare className="w-8 h-8" />
                    </div>
                    <div className="space-y-1">
                      <h3 className="text-xl font-bold text-amber-950 dark:text-amber-200">Pending Verification</h3>
                      <p className="text-xs max-w-lg text-amber-800/90 dark:text-amber-300/80 leading-relaxed">
                        This semestral performance rating is pending supervisor verification. Please review all targets, accomplishments, MOVs, development plans, and supervisor comments before verifying and approving.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={handleVerifyRating}
                      className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 text-xs font-bold shadow-md hover:shadow-lg transition-all cursor-pointer"
                    >
                      <ShieldCheck className="w-4 h-4" />
                      <span>Verify Rating</span>
                    </button>
                  </div>
                ) : (
                  <div className="rounded-2xl border border-emerald-200/80 bg-emerald-50/70 dark:border-emerald-900/50 dark:bg-emerald-950/30 p-6 flex flex-col items-center justify-center text-center space-y-3 shadow-2xs">
                    <div className="p-3 rounded-full bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400 shadow-xs">
                      <ShieldCheck className="w-8 h-8" />
                    </div>
                    <div className="space-y-1">
                      <h3 className="text-xl font-bold text-emerald-950 dark:text-emerald-200">Congratulations! You are now Verified.</h3>
                      <p className="text-xs max-w-lg text-emerald-800/90 dark:text-emerald-300/80 leading-relaxed">
                        {rating.dateVerified ? `Verified by supervisor on ${rating.dateVerified}. ` : ''}
                        This semestral target has been officially verified. Supervisor feedback, development plans, and comments are saved below.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={handleUnverifyRating}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-300 dark:border-emerald-800 bg-white dark:bg-card hover:bg-emerald-50 dark:hover:bg-muted text-emerald-800 dark:text-emerald-200 px-3.5 py-1.5 text-xs font-semibold shadow-2xs transition cursor-pointer"
                    >
                      <RotateCcw className="size-3.5" />
                      <span>Unverify Rating</span>
                    </button>
                  </div>
                )}

                {/* Tabular Areas of Improvement & Development Plan (Inline CRUD) */}
                <div className="space-y-3">
                      ...
                      <table className="w-full text-xs border-collapse text-left">
                        <thead>
                          <tr className="bg-muted/60 font-bold uppercase tracking-wider text-[11px] text-muted-foreground border-b border-border">
                            <th className="px-3 py-2.5 text-center w-[4%] border-r border-border">#</th>
                        ...
                        <tbody className="divide-y divide-border">
                          ...
                          {isAddingArea && (
                            <tr className="bg-emerald-50/40 dark:bg-emerald-950/20 align-top">
                              ...
                              <td className="p-2 border-r border-border">
                                <AutoResizingTextarea
                                  rows={2}
                                  value={newAreaForm.areas_improvement}
                                  onChange={(e) => setNewAreaForm({ ...newAreaForm, areas_improvement: e.target.value })}
                                  placeholder="Enter aim / areas of improvement..."
                                  className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                />
                              </td>
                              <td className="p-2 border-r border-border">
                                <AutoResizingTextarea
                                  rows={2}
                                  value={newAreaForm.development_activities}
                                  onChange={(e) => setNewAreaForm({ ...newAreaForm, development_activities: e.target.value })}
                                  placeholder="Enter development activities..."
                                  className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                />
                              </td>
                              <td className="p-2 border-r border-border">
                                <AutoResizingTextarea
                                  rows={2}
                                  value={newAreaForm.support_resources}
                                  onChange={(e) => setNewAreaForm({ ...newAreaForm, support_resources: e.target.value })}
                                  placeholder="Enter support / resources..."
                                  className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                />
                              </td>
                              <td className="p-2 border-r border-border">
                                <AutoResizingTextarea
                                  rows={2}
                                  value={newAreaForm.progress_intervention}
                                  onChange={(e) => setNewAreaForm({ ...newAreaForm, progress_intervention: e.target.value })}
                                  placeholder="Enter progress / intervention (optional)..."
                                  className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                />
                              </td>
                              <td className="p-2 text-center align-middle">
                                <div className="flex items-center justify-center gap-1">
                                  <button
                                    ...
                                    className="p-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium shadow-2xs transition cursor-pointer disabled:opacity-50"
                                    title="Save new development plan item"
                                  >
                                    {isSavingArea ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />}
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => setIsAddingArea(false)}
                                    className="p-1.5 rounded-lg border border-input bg-background hover:bg-muted text-muted-foreground hover:text-foreground transition cursor-pointer"
                                    title="Cancel"
                                  >
                                    <X className="size-3.5" />
                                  </button>
                          ...
                          {areasOfImprovement.length > 0 ? (
                            areasOfImprovement.map((area, idx) => {
                              ...
                              if (isEditing) {
                                return (
                                  <tr key={area.id || idx} className="bg-primary/5 dark:bg-primary/10 align-top">
                                    <td className="px-2 py-2.5 text-center font-bold text-muted-foreground border-r border-border">
                                      {idx + 1}
                                    </td>
                                    <td className="p-2 border-r border-border">
                                      <AutoResizingTextarea
                                        rows={2}
                                        value={editAreaForm.areas_improvement}
                                        onChange={(e) => setEditAreaForm({ ...editAreaForm, areas_improvement: e.target.value })}
                                        className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                      />
                                    </td>
                                    <td className="p-2 border-r border-border">
                                      <AutoResizingTextarea
                                        rows={2}
                                        value={editAreaForm.development_activities}
                                        onChange={(e) => setEditAreaForm({ ...editAreaForm, development_activities: e.target.value })}
                                        className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                      />
                                    </td>
                                    <td className="p-2 border-r border-border">
                                      <AutoResizingTextarea
                                        rows={2}
                                        value={editAreaForm.support_resources}
                                        onChange={(e) => setEditAreaForm({ ...editAreaForm, support_resources: e.target.value })}
                                        className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                      />
                                    </td>
                                    <td className="p-2 border-r border-border">
                                      <AutoResizingTextarea
                                        rows={2}
                                        value={editAreaForm.progress_intervention}
                                        onChange={(e) => setEditAreaForm({ ...editAreaForm, progress_intervention: e.target.value })}
                                        className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                                      />
                                    </td>
                                    <td className="p-2 text-center align-middle">
                    ...
                    <div className="flex items-center justify-between gap-2 border-b border-border/80 pb-2.5">
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
                          <span>Rater's Comments & Recommendations</span>
                        </h4>
                        <p className="text-[11px] text-muted-foreground">
                    ...
                    <div className="flex items-center justify-between gap-2 border-b border-border/80 pb-2.5">
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                          <span>Strengths & Commendations</span>
                        </h4>
                        <p className="text-[11px] text-muted-foreground">
